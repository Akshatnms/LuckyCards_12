...
namespace Screens
{
    ...
    public class JeetoJokerTimerScreen : ScreenBlueprint
    {
        ...
        int[] CardsBetContiner = new int[12];
        int[] Copyoflastround_CardsBetContiner = new int[12];
        int[] ALL_JACKS_QUEENS_KINGS_BET_CONTINER = new int[3];
        int[] Copyoflastround_ALL_JACKS_QUEENS_KINGS_BET_CONTINER = new int[3];
        ...
        public override ScreenName ScreenID => global::ScreenName.JEETO_JOKER_TIMER_GAME_SCREEN;
        ...
        private void PostBet(Bets data)
        {
            SocketHandler.intance.SendEvent(Constant.OnPlaceBet, data, (res) =>
            {
                ...
                if (response.status == "200") 
                {
                    ...
                    Copyoflastround_ALL_JACKS_QUEENS_KINGS_BET_CONTINER = ALL_JACKS_QUEENS_KINGS_BET_CONTINER;
                    Copyoflastround_CardsBetContiner = CardsBetContiner;
                    balance -= totalBets; isBetConfirmed = true; 
                }
        ...
        IEnumerator ShowWinAmount(int winAmount)
        {
            ...
            winTxt.text = winAmount.ToString();
        }
        ...
        IEnumerator OnRoundEnd(WeelData o)
        {
            ...
            int no = o.win_no;
            var JJW = FindObjectOfType<JeetoJokerWheel>();
            JJW.Spin(no);
            JJW.OnSpinComplete = () =>
            {
        ...
        {
            int J_Q_K_WinIndex = 0;
            if (winNumber < 4) J_Q_K_WinIndex = 0;
            else
            if (winNumber < 8) J_Q_K_WinIndex = 1;
            else
            if (winNumber < 12) J_Q_K_WinIndex = 2;

            int H_S_D_c_WinIndex = 0;
            if (winNumber == 0 || winNumber == 4 || winNumber == 8) H_S_D_c_WinIndex = 0;
            else
            if (winNumber == 1 || winNumber == 5 || winNumber == 9) H_S_D_c_WinIndex = 1;
            else
            if (winNumber == 2 || winNumber == 6 || winNumber == 10) H_S_D_c_WinIndex = 2;
            else
            if (winNumber == 3 || winNumber == 7 || winNumber == 11) H_S_D_c_WinIndex = 3;
            Debug.Log("Win Animation2");
            BettingButtonInteractablity(false);
            for (int i = 0; i < 5; i++)
            {
                cardsBtn[winNumber].interactable = true;
                ALL_JACKS_QUEENS_KINGS_Btn[J_Q_K_WinIndex].interactable = true;
                ...
                ALL_JACKS_QUEENS_KINGS_Btn[J_Q_K_WinIndex].interactable = false;
                yield return new WaitForSeconds(0.5f);
            }
            ALL_HEARTS_SPADES_DIAMONDS_CLUBS_Btn[H_S_D_c_WinIndex].interactable = true;
            ALL_JACKS_QUEENS_KINGS_Btn[J_Q_K_WinIndex].interactable = true;
        ...
        private void ResetAllBets()
        {
            ...
            CardsBetContiner = new int[12];
            ALL_JACKS_QUEENS_KINGS_BET_CONTINER = new int[3];
    ...
    public class Bets
    {
        ...
        public FaceCard faceCards;
        public AllFaceCard allFaceCards;
    }
}
