...
{
    string path = null;
    public Dictionary<string, string> _ticketContent = new Dictionary<string, string>();
    ...
    public void GenerateFile() {
        ...
        using (var fileStream = new FileStream(path, FileMode.OpenOrCreate, FileAccess.Write))
        {
           ...
           Paragraph p = new Paragraph(string.Format("Ticket Id : {0}",12345 )); //iSFSObject.GetUtfString("TICKET_ID"
            ...
            // p = new Paragraph(string.Format("Bet Number : {0}     BetAmount : {1}", 1, 100));
            foreach (KeyValuePair<string, string> ticketInfo in _ticketContent)
            {
                Paragraph m = new Paragraph();
                m.Alignment = Element.ALIGN_CENTER;
                document.Add(m);
                Debug.Log("key  " + ticketInfo.Key + "  value  " + ticketInfo.Value);
                m = new Paragraph(string.Format("Bet Number : {0}     BetAmount : {1}", ticketInfo.Key, ticketInfo.Value ));
                m.Alignment = Element.ALIGN_CENTER;
                document.Add(m);
            }
            document.Close();
